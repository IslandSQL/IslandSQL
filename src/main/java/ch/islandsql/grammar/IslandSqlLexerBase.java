...
public abstract class IslandSqlLexerBase extends Lexer {
    ...
    private boolean prevCharOneOf(String validPrevChars) {
        if (_input.index() < 1) {
            return true;
        }
        String c = _input.getText(Interval.of(_input.index()-1, _input.index()-1));
        return validPrevChars.contains(c);
    }

    public boolean isBeginOfCommand() {
        return prevCharOneOf(" \t\r\n");
    }
